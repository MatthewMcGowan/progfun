...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val other = columnNames.diff(primary :: working)
    columnNames.foldRight[(List[Column], List[Column], List[Column])]((Nil, Nil, Nil)) {
      (name, acc) =>
        if (primary.exists(name.startsWith))
          acc.copy(_1 = new Column(name) :: acc._1)
        else if (working.exists(name.startsWith))
          acc.copy(_2 = new Column(name) :: acc._2)
        else if (other.exists(name.startsWith))
          acc.copy(_3 = new Column(name) :: acc._3)
        else
          acc
    }
  }
